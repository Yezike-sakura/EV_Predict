...
class PDG2Seq_Dncoder(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, time_dim, num_layers=1):
        ...
        self.PDG2Seq_cells.append(PDG2SeqCell(node_num, dim_in, dim_out, cheb_k, embed_dim, time_dim))
        for _ in range(1, num_layers):
            # 后续层的输入应该是上一层的隐藏状态维度 dim_out，而不是最初的 dim_in
            # 否则在 GRU 堆叠时会出现维度不匹配（input_dim + hidden_dim 与 Linear 定义不一致）
            self.PDG2Seq_cells.append(PDG2SeqCell(node_num, dim_out, dim_out, cheb_k, embed_dim, time_dim))
...
class PDG2Seq(nn.Module):
    def __init__(self, args):
        ...
        # Decoder 的输入是真实/预测的目标值 go，其最后一维是 output_dim，
        # 因此这里的 dim_in 应该使用 self.output_dim 而不是原始的 input_dim
        self.decoder = PDG2Seq_Dncoder(args.num_nodes, self.output_dim, args.rnn_units, args.cheb_k,
                                       args.embed_dim, args.time_dim, args.num_layers)
        #predictor
    ...
    def forward(self, source, traget=None, batches_seen=None):
        ...
        # -------------------------------------------------------------
        # 统一约定来自 adapter 的 source/target 布局为:
        # [B, T, N, 3] = [占有率 x, 归一化 t_i_d, d_i_w]
        # 其中:
        #   - t_i_d \in [0, 1)，内部再 *288 取整后作为时间索引
        #   - d_i_w \in {0,...,6} 表示星期索引
        # -------------------------------------------------------------
        # 🎯 修复维度爆炸：全市共享相同时间，仅需提取第 0 个节点的特征进行降维 [B, T]
        # 编码端时间索引
        t_i_d_data1 = source[:, :, 0, 1]
        # 解码端时间索引
        t_i_d_data2 = traget[:, :, 0, 1]

        # 根据 time-of-day 构造时间嵌入
